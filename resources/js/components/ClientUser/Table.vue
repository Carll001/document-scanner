...
const goToPage = (pageNum: number) => {
  ...
  if (props.filters.role) query.role = props.filters.role
    ...
    <Table>
      ...
      <TableBody>
        <TableRow v-for="(user, index) in users" :key="user.id">
          <TableCell>{{ index + props.pagination.from }}</TableCell>
          ...
          <TableCell class="space-x-2">
            <Button
              size="sm"
              @click="router.visit(usersRoutes.show.url({ user: user.id }))"
            >
              View
            </Button>

            <Button size="sm" @click="selectedUser = user">Edit</Button>

            <Button size="sm" variant="destructive" @click="userToDelete = user">
              Delete
            </Button>
          </TableCell>
        </TableRow>
